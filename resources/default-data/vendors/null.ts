...
const imageRequest = async (_config: ImageConfig, _model: ImageModel): Promise<string> => {
  throw new Error("空模板暂未实现 imageRequest，请接入真实图片生成接口后再启用图片模型");
};

const videoRequest = async (_config: VideoConfig, _model: VideoModel): Promise<string> => {
  throw new Error("空模板暂未实现 videoRequest，请接入真实视频生成接口后再启用视频模型");
};

const ttsRequest = async (_config: TTSConfig, _model: TTSModel): Promise<string> => {
  throw new Error("空模板暂未实现 ttsRequest，请接入真实语音生成接口后再启用 TTS 模型");
};
 ...
 *     未实现的适配器函数必须显式 throw，不可返回空字符串，也不可省略导出。
 ...
 *   5. 仅实现用户需要的模型类型，未用到的函数必须显式 throw。
