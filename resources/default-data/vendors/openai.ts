...
const imageRequest = async (_config: ImageConfig, _model: ImageModel): Promise<string> => {
  throw new Error("OpenAI adapter 暂未实现 imageRequest，请接入真实图片生成接口后再启用图片模型");
};
const videoRequest = async (_config: VideoConfig, _model: VideoModel): Promise<string> => {
  throw new Error("OpenAI adapter 暂未实现 videoRequest，请接入真实视频生成接口后再启用视频模型");
};
const ttsRequest = async (_config: TTSConfig, _model: TTSModel): Promise<string> => {
  throw new Error("OpenAI adapter 暂未实现 ttsRequest，请接入真实语音生成接口后再启用 TTS 模型");
};
const checkForUpdates = async (): Promise<{ hasUpdate: boolean; latestVersion: string; notice: string }> => {
  return { hasUpdate: false, latestVersion: "2.0", notice: "" };
};
const updateVendor = async (): Promise<string> => {
  throw new Error("OpenAI adapter 暂未提供自动更新");
};
// ============================================================
// 导出
// ============================================================
