...
const textRequest = (_model: TextModel, _think: boolean, _thinkLevel: 0 | 1 | 2 | 3) => {
  throw new Error("Volcengine SD2 adapter 暂未实现 textRequest，请接入真实文本接口后再启用文本模型");
};

const imageRequest = async (_config: ImageConfig, _model: ImageModel): Promise<string> => {
  throw new Error("Volcengine SD2 adapter 暂未实现 imageRequest，请接入真实图片生成接口后再启用图片模型");
};
...
const ttsRequest = async (_config: TTSConfig, _model: TTSModel): Promise<string> => {
  throw new Error("Volcengine SD2 adapter 暂未实现 ttsRequest，请接入真实语音生成接口后再启用 TTS 模型");
};
...
const updateVendor = async (): Promise<string> => {
  throw new Error("Volcengine SD2 adapter 暂未提供自动更新");
};
